 ...
 * `ProviderInstanceId` in `@multi/contracts`.
 */
...
export function AddProviderInstanceDialog({ open, onOpenChange }: AddProviderInstanceDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <AddProviderInstanceDialogContent
        key={open ? "provider-instance-dialog-open" : "provider-instance-dialog-closed"}
    ...
    <DialogPopup className="max-w-xl overflow-hidden">
      <div className="flex min-h-0 flex-col overflow-hidden border-foreground/10 bg-background shadow-2xl">
        ...
        >
          ...
          <label className={cn("grid gap-2", wizardStep !== 1 && "hidden")} htmlFor={labelInputId}>
          ...
          <label
            className={cn("grid gap-2", wizardStep !== 1 && "hidden")}
            htmlFor={instanceIdInputId}
          >
