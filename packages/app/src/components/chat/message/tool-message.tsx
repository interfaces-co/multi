...
function SubagentStatusSurface({ subagents }: { subagents: ReadonlyArray<WorkLogSubagent> }) {
  return (
    <div className="agent-panel-meta-agent-chat__nested-subagents mt-1 max-h-80 w-full overflow-x-hidden overflow-y-auto pl-5 text-detail">
      {subagents.map((subagent) => (
        <div
          key={subagent.providerThreadId ?? subagent.threadId ?? subagent.agentId}
          className="agent-panel-meta-agent-chat__nested-subagent group/subagent flex min-h-6 w-fit max-w-full items-center gap-1 overflow-hidden"
