...
import { type ProviderDriverKind } from "@multi/contracts";

import { PROVIDER_ICON_BY_PROVIDER } from "./provider-icon-utils";
...
export const ProviderInstanceIcon = memo(function ProviderInstanceIcon(props: {
  ...
  return (
    ...
    >
      {Icon ? (
        <Icon className={cn("shrink-0", props.iconClassName ?? "size-5")} aria-hidden />
      ) : (
      ...
      {props.showBadge ? (
        <span
          className={cn(
            ...
            props.accentColor
              ? "bg-(color:--provider-accent) text-white"
