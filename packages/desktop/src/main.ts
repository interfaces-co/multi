...
const BASE_DIR = process.env.MULTI_HOME?.trim() || Path.join(OS.homedir(), ".multi");
...
function resolveAdvertisedHostOverride(): string | undefined {
  const override = process.env.MULTI_DESKTOP_LAN_HOST?.trim();
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { multiCommitHash?: unknown };
    return normalizeCommitHash(parsed.multiCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.MULTI_COMMIT_HASH);
  if (envCommitHash) {
...
function resolveBackendEntry(): string {
  return Path.join(resolveAppRoot(), "packages/server/dist/bin.mjs");
}
...
function resolveDesktopStaticDir(): string | null {
  ...
  const candidates = [
    Path.join(appRoot, "packages/server/dist/client"),
    Path.join(appRoot, "packages/app/dist"),
  ];
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Multi failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function registerDesktopProtocol(): void {
  ...
  if (!staticRoot) {
    throw new Error(
      "Desktop static bundle missing. Build packages/server (with bundled client) first.",
    );
  }
...
function handleCheckForUpdatesMenuClick(): void {
  const hasUpdateFeedConfig =
    readAppUpdateYml() !== null || Boolean(process.env.MULTI_DESKTOP_MOCK_UPDATES);
  const disabledReason = getAutoUpdateDisabledReason({
    isDevelopment,
    isPackaged: app.isPackaged,
    platform: process.platform,
    appImage: process.env.APPIMAGE,
    disabledByEnv: process.env.MULTI_DISABLE_AUTO_UPDATE === "1",
    hasUpdateFeedConfig,
  });
  if (disabledReason) {
...
function configureAutoUpdater(): void {
  ...
  if (process.env.MULTI_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.MULTI_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
...
async function bootstrap(): Promise<void> {
  ...
  const configuredBackendPort = resolveConfiguredDesktopBackendPort(process.env.MULTI_PORT);
  if (isDevelopment && configuredBackendPort === undefined) {
    throw new Error("MULTI_PORT is required in desktop development.");
  }
