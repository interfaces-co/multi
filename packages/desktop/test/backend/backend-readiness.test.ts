...
describe("waitForHttpReady", () => {
  ...
  it("can use a backend-owned readiness path when the root route redirects", async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(JSON.stringify({ environmentId: "env-test" }), {
        status: 200,
    ...
    await waitForHttpReady("http://127.0.0.1:3773", {
      ...
      timeoutMs: 1_000,
      intervalMs: 0,
      path: "/.well-known/multi/environment",
    });

    expect(fetchImpl).toHaveBeenCalledTimes(1);
    expect(fetchImpl).toHaveBeenCalledWith(
      "http://127.0.0.1:3773/.well-known/multi/environment",
      expect.objectContaining({ redirect: "manual" }),
    );
  });
});
