...
import type { TerminalManagerShape } from "../../src/terminal/Manager.service";
import {
  ...
  type PtySpawnInput,
  PtySpawnError,
} from "../../src/terminal/PTY.service";
import { makeTerminalManagerWithOptions } from "../../src/terminal/Manager";
...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "multi-terminal-" });
...
it.layer(NodeServices.layer, { excludeTestServices: true })("TerminalManager", (it) => {
  ...
  it.effect("starts terminals at the git worktree root when opened from a subdirectory", () =>
    Effect.gen(function* () {
      ...
      const nestedCwd = path.join(repoRoot, "packages", "server");
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      assert.equal(spawnInput.env.MULTI_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.MULTI_WORKTREE_PATH, "/repo/worktree-a");
