...
import { ServerEnvironment } from "../../src/environment/ServerEnvironment.service.ts";
import { ServerEnvironmentLive } from "../../src/environment/ServerEnvironment.ts";
...
it.layer(NodeServices.layer)("ServerEnvironmentLive", (it) => {
  it.effect("persists the environment id across service restarts", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "multi-server-environment-test-",
      });
  ...
  it.effect("fails instead of overwriting a persisted id when reading the file errors", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "multi-server-environment-read-error-test-",
      });
