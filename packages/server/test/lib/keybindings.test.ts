import { KeybindingCommand, KeybindingRule, KeybindingsConfig } from "@multi/contracts";
...
import { KeybindingsConfigError } from "@multi/contracts";
...
const readKeybindingsConfig = (configPath: string) =>
  Effect.gen(function* () {
    ...
    return yield* decodeUnknownKeybindingsConfigJson(rawConfig);
  });
...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("formats invalid resolved keybinding rules with the custom message", () =>
    Effect.sync(() => {
      const result = decodeUnknownResolvedKeybindingFromConfigExit({
        key: "mod+shift+d+o",
        command: "terminal.new",
      });
