...
import { increment, terminalRestartsTotal, terminalSessionsTotal } from "../observability/Metrics";
...
function resolveShellCandidates(
  ...
  const loginShellCandidates = listLoginShellCandidates(
    process.platform,
    requested?.shell,
    userLoginShellResolver(),
  ).map((shell) => shellCandidateFromCommand(normalizeShellCommand(shell)));
...
async function resolveGitWorkTreeRoot(cwd: string): Promise<string | null> {
...
export const makeTerminalManagerWithOptions = Effect.fn("makeTerminalManagerWithOptions")(
  function* (options: TerminalManagerOptions) {
    ...
    const trySpawn = Effect.fn("terminal.trySpawn")(function* (
      ...
      const candidateEnv =
        process.platform === "win32" ? spawnEnv : { ...spawnEnv, SHELL: candidate.shell };
    ...
    const open: TerminalManagerShape["open"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const cwd = yield* resolveTerminalCwd(input.cwd);
