...
import { OrchestrationEventStore } from "../persistence/OrchestrationEventStore.service.ts";
import { ProjectionPendingApprovalRepository } from "../persistence/ProjectionPendingApprovals.service.ts";
import { ProjectionProjectRepository } from "../persistence/ProjectionProjects.service.ts";
...
import { ProjectionStateRepositoryLive } from "../persistence/ProjectionState.ts";
import { ProjectionThreadActivityRepositoryLive } from "../persistence/ProjectionThreadActivities.ts";
import { ProjectionThreadMessageRepositoryLive } from "../persistence/ProjectionThreadMessages.ts";
import { ProjectionThreadProposedPlanRepositoryLive } from "../persistence/ProjectionThreadProposedPlans.ts";
      ...
      switch (event.type) {
        ...
        case "project.meta-updated": {
          ...
          yield* projectionProjectRepository.upsert({
            ...
            ...(event.payload.projectRoot !== undefined
              ? { projectRoot: event.payload.projectRoot }
              : {}),
        ...
        case "thread.reverted": {
          ...
          const retainedMessageIds = new Set<string>(
            existingTurns
              ...
              .flatMap((turn) => [turn.pendingMessageId, turn.assistantMessageId])
              .flatMap((messageId) => (messageId === null ? [] : [String(messageId)])),
          );
          const keptRows = retainMessagesAfterCheckpointRevert({
            messages: existingRows,
            retainedTurnIds,
            retainedMessageIds,
    ...
    const applyThreadProposedPlansProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.reverted": {
          ...
          const keptRows = retainTurnFactsAfterCheckpointRevert(
            existingRows,
            getCheckpointRetainedTurnIds(existingTurns, event.payload.turnCount),
          );
          if (keptRows.length === existingRows.length) {
            return;
          }
