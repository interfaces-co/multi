...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__MULTI_ENV_PATH_START__\n/a:/b\n__MULTI_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__MULTI_ENV_PATH_START__");
    expect(args?.[1]).toContain("__MULTI_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__MULTI_ENV_PATH_END__",
        "__MULTI_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__MULTI_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      ["__MULTI_ENV_CUSTOM_VAR_START__", "  padded value  ", "__MULTI_ENV_CUSTOM_VAR_END__"].join(
        "\n",
      ),
    );
