...
function readBrowserBridge(): DesktopBrowserBridge | null {
  return window.desktopBridge?.browser ?? null;
}
...
export function BrowserPanel({ mode, threadId, onClosePanel }: BrowserPanelProps) {
  ...
  useEffect(() => {
    ...
    void runBrowserAction(() => api.open({ threadId })).then((state) => {
      if (cancelled) {
        return;
      }
      if (!state) {
  ...
  useLayoutEffect(() => {
    ...
    const syncBounds = () => {
      ...
      void runBrowserAction(() => api.setPanelBounds({ threadId, bounds }));
    };
  ...
  const onSubmitAddress = useCallback(() => {
    ...
    void runBrowserAction(() =>
      api.navigate({ threadId, tabId: activeTab.id, url: normalizedAddress }),
    ).then((state) => {
      if (state) {
        upsertThreadState(state);
      }
  ...
  const onChooseSuggestion = useCallback(
    (suggestion: BrowserAddressSuggestion) => {
      ...
      if (suggestion.kind === "tab" && typeof tabId === "string") {
        void runBrowserAction(() => api.selectTab({ threadId, tabId })).then((state) => {
          if (state) {
            upsertThreadState(state);
          }
  ...
  const onCreateTab = useCallback(() => {
    ...
    void runBrowserAction(() => api.newTab({ threadId, activate: true })).then((state) => {
      if (state) {
        upsertThreadState(state);
      }
      window.requestAnimationFrame(() => {
  ...
  const onCloseTab = useCallback(
    (tabId: string) => {
      ...
      void runBrowserAction(() => api.closeTab({ threadId, tabId })).then((state) => {
        if (!state) {
          return;
        }
        upsertThreadState(state);
        ...
        <div className="flex shrink-0 items-center gap-1 [-webkit-app-region:no-drag]">
          <Button
            ...
            onClick={() => {
              ...
              void runBrowserAction(() => api.goBack({ threadId, tabId: activeTab.id })).then(
                (state) => {
                  if (state) {
                    upsertThreadState(state);
                  }
          ...
          <Button
            ...
            onClick={() => {
              if (!api || !activeTab) return;
              void runBrowserAction(() => api.goForward({ threadId, tabId: activeTab.id })).then(
                (state) => {
                  if (state) {
                    upsertThreadState(state);
                  }
          ...
          <Button
            ...
            onClick={() => {
              if (!api || !activeTab) return;
              void runBrowserAction(() => api.reload({ threadId, tabId: activeTab.id })).then(
                (state) => {
                  if (state) {
                    upsertThreadState(state);
                  }
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      void runBrowserAction(() => api.selectTab({ threadId, tabId: tab.id })).then(
                        (state) => {
                          if (state) {
                            upsertThreadState(state);
                          }
