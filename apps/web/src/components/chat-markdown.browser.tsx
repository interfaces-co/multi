...
describe("chat-markdown", () => {
  ...
  it("disambiguates duplicate file basenames inline", async () => {
    const firstPath = "/Users/yashsingh/p/multi/apps/web/src/components/chat/messages-timeline.tsx";
    const secondPath = "/Users/yashsingh/p/multi/apps/web/src/components/messages-timeline.tsx";
    const screen = await render(
      <ChatMarkdown
    ...
    try {
      await expect
        .element(page.getByRole("link", { name: "messages-timeline.tsx · components/chat" }))
        .toBeInTheDocument();
      await expect
        .element(page.getByRole("link", { name: "messages-timeline.tsx · src/components" }))
        .toBeInTheDocument();
    } finally {
      await screen.unmount();
    }
