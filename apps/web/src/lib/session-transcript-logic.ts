...
export const HIDDEN_ACTIVITY_KINDS = new Set<OrchestrationThreadActivity["kind"]>([
...
function stage(kind: string): number {
...
function sortActs(
  ...
  return [...activities].toSorted((left, right) => {
    ...
    const date = left.createdAt.localeCompare(right.createdAt);
    if (date !== 0) {
      return date;
    }
    const kind = stage(left.kind) - stage(right.kind);
    if (kind !== 0) {
      return kind;
    }
...
export function derivePendingApprovals(
  ...
  const ordered = sortActs(activities);
...
export function shouldShowActivity(activity: OrchestrationThreadActivity): boolean {
  if (HIDDEN_ACTIVITY_KINDS.has(activity.kind)) {
    return false;
  }
