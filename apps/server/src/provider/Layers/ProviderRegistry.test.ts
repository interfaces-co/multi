...
import { ServerSettingsService, type ServerSettingsShape } from "../../server-settings";
    ...
    describe("checkCodexProviderStatus", () => {
      ...
      it.effect("returns unavailable when codex is below the minimum supported version", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            status.message,
            "Codex CLI v0.36.0 is too old for Multi. Upgrade to v0.37.0 or newer and restart Multi.",
          );
        }).pipe(
          Effect.provide(
            mockSpawnerLayer((args) => {
