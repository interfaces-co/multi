...
Use this when you want to connect to a Multi server from another device such as a phone, tablet, or separate desktop app.
...
Run the server with `multi serve`.

```bash
npx usemulti serve --host "$(tailscale ip -4)"
```

`multi serve` starts the server without opening a browser and prints:
...
Use `multi serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `multi project ...` on the server machine instead.
...
1. `multi serve` issues a one-time owner pairing token.
...
Use `multi auth` to manage access after the initial pairing flow.
...
Use `multi auth --help` and the nested subcommand help pages for the full reference.
...
- Use `multi auth` to revoke credentials or sessions you no longer trust.
