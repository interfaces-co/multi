...
Multi has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.multi/userdata/logs/server.trace.ndjson`).
...
```bash
export MULTI_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export MULTI_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export MULTI_OTLP_SERVICE_NAME=multi-local
```

Optional:

```bash
export MULTI_TRACE_MIN_LEVEL=Info
export MULTI_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `MULTI_OTLP_*`.

macOS app bundle example:

```bash
MULTI_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
MULTI_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
MULTI_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/Multi.app/Contents/MacOS/Multi"
```
...
```bash
tail -f "$MULTI_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$MULTI_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$MULTI_HOME/userdata/logs/server.trace.ndjson"
```
...
- `MULTI_OTLP_TRACES_URL` was not set
...
- `MULTI_TRACE_FILE`: override trace file path
- `MULTI_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `MULTI_TRACE_MAX_FILES`: rotated file count, default `10`
- `MULTI_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `MULTI_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `MULTI_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `MULTI_OTLP_TRACES_URL`: OTLP trace endpoint
- `MULTI_OTLP_METRICS_URL`: OTLP metric endpoint
- `MULTI_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `MULTI_OTLP_SERVICE_NAME`: service name, default `multi-server`
