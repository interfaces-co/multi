...
| Compact toolbar              | Cursor base compact toolbar participates through prompt-input selectors and has compact-specific display rules; model picker has explicit order/margins | Multi keeps `ui-prompt-input-toolbar`, left, and right as real flex wrappers with additional Tailwind classes                                      |
