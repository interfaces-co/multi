...
- Server orchestration: `packages/server/src/orchestration/**`, `packages/server/src/provider/**`
...
| Planning mode                                      | Partial                                              | `ProviderInteractionMode` has `plan`; Codex plan-mode developer instructions exist in `packages/server/src/provider/CodexDeveloperInstructions.ts`; proposed plans are stored/rendered via `OrchestrationProposedPlan` and `ProposedPlanCard` |
