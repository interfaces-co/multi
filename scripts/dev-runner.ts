...
export const DEFAULT_MULTI_HOME = Effect.map(Effect.service(Path.Path), (path) =>
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("MULTI_PORT_OFFSET"),
  devInstance: optionalStringConfig("MULTI_DEV_INSTANCE"),
});
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_MULTI_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly multiHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    if (!isDesktopMode && host !== undefined) {
      output.MULTI_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.MULTI_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.MULTI_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.MULTI_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.MULTI_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.MULTI_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.MULTI_LOG_WS_EVENTS;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read MULTI_PORT_OFFSET/MULTI_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  multiHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Multi data (equivalent to MULTI_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("MULTI_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to MULTI_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("MULTI_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to MULTI_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("MULTI_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to MULTI_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("MULTI_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to MULTI_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("MULTI_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to MULTI_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("MULTI_PORT")),
  ),
