...
for f in packages/app/src/routes/*.tsx packages/app/src/app/routes/*.tsx packages/app/src/diff-route-search.ts packages/app/src/thread-routes.ts packages/app/src/chat-route-persistence.ts; do wc -l "$f"; done
```

Current facts:

- [x] `packages/app/src/routes` has `11` TanStack route files.
- [x] `packages/app/src/app/routes` has `6` route view files.
...
- [x] `packages/app/src/diff-route-search.ts` is `76` lines.
- [x] `packages/app/src/thread-routes.ts` is `59` lines.
- [x] `packages/app/src/chat-route-persistence.ts` is `78` lines.
...
- [ ] TanStack route files own path params, search validation, and simple
      route-level redirects only.
- [ ] TanStack route files do not own composer/model/plan/git/sidebar policy.
- [ ] Search schemas stay near route ownership and are imported by components
...
- [x] `packages/app/src/app/routes/chat-thread-route.tsx`: validates thread
      ...
      promoted drafts.
- [x] `packages/app/src/app/routes/chat-route.tsx`: chat layout route, command
      palette close behavior, and hardcoded Escape listener pending keybinding
      cleanup.
- [x] `packages/app/src/app/routes/settings-route.tsx`: settings layout route
      and hardcoded Escape back listener pending keybinding cleanup.
...
- [ ] Route views may bridge route state to client-only stores.
- [ ] Route views may own global listeners only when the listener is route
      scoped and configured through keybindings where it is product behavior.
- [ ] Route views should call shell/composer/model/plan owners instead of
...
- [x] `packages/app/src/diff-route-search.ts`: shared chat shell search parser
      for diff and workbench params. Current callers are chat routes, diff panel,
      chat view, and Git panel.
- [x] `packages/app/src/thread-routes.ts`: route param builders/resolvers for
      server threads and draft routes. Current callers include route views, toast,
      command palette, thread actions, shell host, and chat view.
- [x] `packages/app/src/chat-route-persistence.ts`: local-storage persistence
      for the last chat route target. Current callers include chat route views,
      shell host, and sidebar footer.

Target:

- [ ] Move `diff-route-search.ts` under route ownership and rename it for chat
      shell search if the shared search shape stays.
- [ ] Inline `diff-route-search.ts` only if diff/workbench search becomes
      panel-local and routes no longer share it.
- [ ] Keep `thread-routes.ts` while multiple surfaces need a canonical route
      target type and route param builders.
- [ ] Move `chat-route-persistence.ts` under app route persistence ownership;
...
- [ ] Search params must remain serializable primitives at the route boundary.
- [ ] Workbench tab search belongs to the shell/workbench route contract, not
      the plan or terminal component.
- [ ] Diff search belongs to the chat route only while diff is a route-level
      panel state.

## First Route Cleanup Candidates

- [ ] Move hardcoded Escape behavior in `chat-route.tsx` and
      `settings-route.tsx` into configurable keybindings.
- [ ] Move `diff-route-search.ts`, `thread-routes.ts`, and
...
- [ ] Keep `root-route.tsx` as app bootstrap until environment runtime startup
      has a clearer owner.

## Done Means

- [ ] Route files remain thin path/search/view selectors.
- [ ] Route view effects are classified in
      [react-effect-callsite-inventory.md](./react-effect-callsite-inventory.md).
- [ ] Search contracts are colocated with route ownership.
- [ ] Keybindings are configurable for route-level shortcuts.
- [ ] `routeTree.gen.ts` is generated from the kept route files.
